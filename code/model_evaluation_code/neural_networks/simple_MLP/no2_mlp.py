...
def get_plot(model, x_val, y_val):
    ...
    plt.ylabel("NO2", fontsize = 13)
    plt.xticks(size = 10)
    plt.yticks(size = 10)
    plt.title("Neural Network : NO2 scatter plot", fontsize = 15)
...
'''model structure :(Three hidden layers having 100 neurons for each,
activation function for all hidden layers : rectifier(relu),
epoch value: 150, optimizer function : lbfgs, learning rate : 0.009 and batch size : 28)'''

nn = MLPRegressor(hidden_layer_sizes=(100, 100, 100 ), activation='relu', max_iter= 150, solver='lbfgs', n_iter_no_change=100, learning_rate_init=0.009, batch_size=28)
